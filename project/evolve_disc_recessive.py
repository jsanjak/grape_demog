...
def main(rep):
    ...
    p_anc = {'demography':np.array([N_anc]*Gen_burnin,dtype=np.uint32),
       'nregions':[fp11.Region(0,1,1)],
        ...
        'sregions':[fp11.GammaS(0,1,1,-0.05,0.3,0)],
        'rates':((11./12.)*theta/float(4*N_anc),(1./12.)*theta/float(4*N_anc),rho/float(4*N_anc))
    }

    # Bottle neck parameters
    p_bottle ={'demography':np.array([N_pre]*Gen_bottle,dtype=np.uint32),
       'nregions':[fp11.Region(0,1,1)],
        'recregions':[fp11.Region(0,1,1)],
        'sregions':[fp11.GammaS(0,1,1,-0.05,0.3,0)],
        'rates':((11./12.)*theta/float(4*N_anc),(1./12.)*theta/float(4*N_anc),rho/float(4*N_anc))
    ...
    output = open('data/disc_recessive/disc_recessive_clonal_grape.'+str(rep)+'.pkl', 'wb')
